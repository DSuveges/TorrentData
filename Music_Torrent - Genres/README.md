...
But on the other hand, I started to wonder what users on the biggest Hungarian torrent site are listening to. Which genres do they prefer? In this project, I analized the distribution of genres and the number of downloads for different genres. The task was complicatd by the fact that the genre field of the torrent data sheet is not strictly defined, it is up to the user to fill that field upon uploading, or even they can completely omit it as well. This yielded an extremely diverse set of variable with over 2200 separate values.

**The analysis can be divided into the following steps:**

1. **Parse genre data:** during the torrent data sheet processing, the genre information is stored in one field of a csv file, where each genre is separated by an underscore. In this step, the torrent data file is read by a Perl script, that separates the genre fields (as more genres can be associated with one music torrent), and saves it paired with the torrent ID in a new csv file.
...
3. **Summarizing genre data:** the number of torrent files and downloads are summarized over the associated genres. In the resulting table each row represents one genre.
...
Based on the report generated by the commands listed above, the most frequently used 200 genre definitions cover 93% of the torrents, and 90% of the 140,000 genre assignment.

![The usage of the top 200 most frequently used genre definitions](./figure/Top200barplot.png)

The barplot shows that there are only a few very frequently used genres, while most genres in the Top 200 are already quite rare. So the Top 200 definitions cover the dataset well, though 200 definitions are still way too many to visualize and also has incosistentencies.

I have developed a genre mask to fix the most commonly occuring typos, the mask omitted non-real genre definitions ("192kb", "80s", "live" etc.),  pooled closely related genres like "nordic metal" and "viking metal". Finally, the genre mask had 203 entries that restricted the genre set to only 42 definitions (see: *mask_definitions* and *Tag_cleaner.pl*) that were further grouped into five categories: "rock", "electronic", "pop", "hip-hop", "other".
...
Before visualization, I assigned colors to each genre that makes it easier to identify them on various graphs. There are base colors that are selected for categories then a color gradient is generated from the base color to white based on the count of the given genre.
...
To get a qualitative overview of how the number of torrents and downloads are distributed among main categories and further genres, I used **treemap** which is especially useful to visualize hierarchical data, where the area of a category (*leaf*) is proportional to its relative value, and these areas are further grouped.

![Treemap of music genres](./figure/treemap_.png)

These graphs reveal that the highest number of torrents do not necessarily correspond to the highest number of downloads. As we can see, while the most frequently uploaded genres belong to the **Electronic** category, the most frequently downloaded torrents belong to **rock**. To further quantify these differences, I created a barchart on which I display the number of torrents, downloads and average downloads for each genre.

![Barplot of torrent genre usage](./figure/barplot_color.png)

Interestingly, some genres have a very high average download number, though only a small number of torrents are available. Two of these genres are marked with red asterisks:

1. In the rock category, the genre called **nemzeti-rock** refers to a special kind of rock music whose lyrics have a nationalistic atmosphere.
2. In the other category, the genre called **mulatos** refers to a special Hungarian kind of music often listen to at big wedding parties.

Music pieces belonging to these genres are fullfilling a special need of a relatively narrow audience. Because these pieces are hard to come by, the few that are uploaded become very popular. The nationality bias I have determined with my other analysis ([link](https://github.com/DSuveges/TorrentData/tree/master/Music_Torrent%20-%20Nationality)) holds especially well here.
