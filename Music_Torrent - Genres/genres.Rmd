...
To make the genre definition list more concise, I applied an iterative process, with the combination of Perl scripts and R commands, I constructed a genre mask set to fix the most frequently made typo-s and inconsistent defintions. In this process I selected the first 100 most commonly used genres and manually processed them to get rid of non-meaningful words and typos. I also pooled closely related genres like "nordic metal" and "viking metal". Then I repeated this process where newer and newer definitions come up to the top 100. 
...
At the end of the optimization, I have developed a genre mask with 203 entries that restricted the genre set to only 42 definitions (see: mask_definitions.txt and Tag_cleaner.pl) that were grouped into five categories: "rock", "electronic", "pop", "hip-hop", "other".

__Let's show a list of the final genre definitions:__
...
cat("Number of genre definitions:", length(Genre_Table),
    "\nNumber of torrents covered:", length(ID_Table), "That is", round(length(ID_Table)/TorrentNumber *100,1), "percent of the total torrent population.\n There is", nrow(CleanGenre), "assignemnts, that cover", round(nrow(CleanGenre)/AssignedGenres *100, 1), "percent of the original assignments")
```
...
DownloadCount <- sapply(CleanGenre_small$ID, function(x) Torrent_df[ Torrent_df$TorrentID == x , "Downloaded"])
...
TorrentCount <- sapply(GenreDf$genres, function(x) sum(CleanGenre_small[ CleanGenre_small$genre == x, "TorrentCount"]))
GenreDf$Count <- TorrentCount

# calculte how many downloads belong to each genre definition:
TorrentDownloads <- sapply(GenreDf$genres, function(x) sum(CleanGenre_small[ CleanGenre_small$genre == x, "Downloads"]))
